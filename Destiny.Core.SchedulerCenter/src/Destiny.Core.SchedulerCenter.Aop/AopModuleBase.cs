...
namespace Destiny.Core.SchedulerCenter.Aop
{
    /// <summary>
    /// 全局AOP模块
    /// </summary>
            ...
            if (typs?.Length > 0)
            {
                ...
                foreach (var item in typs)
                {
                    ...
                    service.ConfigureDynamicProxy(cof =>
                    {
                        ...
                        if (Enabled)
                            cof.Interceptors.AddTyped(item, Predicates.ForNameSpace(InterceptorsModule)/*,Predicates.ForNameSpace(IInterceptorsModule)*/);////这种是配置只需要代理的层, Predicates.ForNameSpace("Destiny.Core.SchedulerCenter.Application.Contracts")
